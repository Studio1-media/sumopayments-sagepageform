<?php
    ...
    if( class_exists( 'WC_Payment_Gateway_CC' ) ) {
        class _WC_Gateway_SUMO_Sagepay_Direct extends WC_Payment_Gateway_CC {}
    } else {
        class _WC_Gateway_SUMO_Sagepay_Direct extends WC_Payment_Gateway {}
    }

    class WC_Gateway_SUMO_Sagepay_Direct extends _WC_Gateway_SUMO_Sagepay_Direct {
        ...
        public function __construct() {
            ...
            $this->id                   = 'sumosagepaydirect';
            $this->method_title         = __( 'SUMO SagePay Direct', 'sumopayments_sagepayform' );
            $this->method_description   = __( 'SUMO SagePay Direct', 'sumopayments_sagepayform' );
			...
			$this->sage_cardtypes = apply_filters( 'wc_sagepaydirect_cardtypes', array(
        		'MasterCard'		=> __( 'MasterCard Credit', 'sumopayments_sagepayform' ),
				'MasterCard Debit'	=> __( 'MasterCard Debit', 'sumopayments_sagepayform' ),
				'Visa'				=> __( 'Visa Credit', 'sumopayments_sagepayform' ),
				'Visa Debit'		=> __( 'Visa Debit', 'sumopayments_sagepayform' ),
				'Discover'			=> __( 'Discover', 'sumopayments_sagepayform' ),
				'American Express' 	=> __( 'American Express', 'sumopayments_sagepayform' ),
				'Maestro'			=> __( 'Maestro', 'sumopayments_sagepayform' ),
				'JCB'				=> __( 'JCB', 'sumopayments_sagepayform' ),
				'Laser'				=> __( 'Laser', 'sumopayments_sagepayform' ),
			) );
			...
			// 3D iframe
            $this->iframe_3d_callback   = esc_url( SUMO_SAGEPLUGINURL . 'assets/pages/3dcallback.php' );
            $this->iframe_3d_redirect   = esc_url( SUMO_SAGEPLUGINURL . 'assets/pages/3dredirect.php' );
			...
			if ( $this->status != 'live' ) {
				$this->description .= ' ' . sprintf( __( '<br />TEST MODE ENABLED.<br />In test mode, you can use Visa card number 4929000000006 with any CVC and a valid expiration date or check the documentation (<a href="%s">Test card details for your test transactions</a>) for more card numbers.', 'sumopayments_sagepayform' ), 'http://www.sagepay.co.uk/support/12/36/test-card-details-for-your-test-transactions' );
		...
		public function payment_fields() {
			...
			if ( is_add_payment_method_page() ) {
				$pay_button_text = __( 'Add Card', 'sumopayments_sagepayform' );
			} else {
				$pay_button_text = '';
			}
			...
			try {
				...
				if ( $sage_card_token === false || $sage_card_token === 'new' ) {
					...
					if ( empty( $sage_card_type ) || ctype_digit( $sage_card_type ) || !in_array($sage_card_type, $this->cardtypes ) ) {
						throw new Exception( __( 'Please choose a card type', 'sumopayments_sagepayform' ) );
					}

					if ( ( $this->cvv && !ctype_digit( $sage_card_cvc ) ) || ( $this->cvv && strlen( $sage_card_cvc ) < 3 ) || ( $this->cvv && strlen( $sage_card_cvc ) > 4 ) ) {
						throw new Exception( __( 'Card security code is invalid (only digits are allowed)', 'sumopayments_sagepayform' ) );
					}

					if ( !ctype_digit( $sage_card_exp_month ) || $sage_card_exp_month > 12 || $sage_card_exp_month < 1 ) {
						throw new Exception( __( 'Card expiration month is invalid', 'sumopayments_sagepayform' ) );
					}

					if ( !ctype_digit( $sage_card_exp_year ) || $sage_card_exp_year < date('y') || strlen($sage_card_exp_year) != 2 ) {
						throw new Exception( __( 'Card expiration year is invalid', 'sumopayments_sagepayform' ) );
					}

					if ( empty( $sage_card_number ) || ! ctype_digit( $sage_card_number ) ) {
						throw new Exception( __( 'Card number is invalid', 'sumopayments_sagepayform' ) );
					}
		...
		public function process_payment( $order_id ) {
			...
			// woocommerce order instance
           	...
           	if( $order->get_total() == 0 ) {
				...
				if( empty($data) ) {
					$this->sagepay_message( (__('Payment error, please try again', 'sumopayments_sagepayform') ) , 'error' );
				} else {
					...
					if( isset($sageresult['Status']) && $sageresult['Status']!= '' ) {
	            	     ...
	            	     * Payment Failed! - $sageresult['Status'] is empty
	            	  	 */
						$order->add_order_note( __('Payment failed, contact Sage. This transaction returned no status, you should contact Sage.', 'sumopayments_sagepayform') );

						$this->sagepay_message( (__('Payment error, please contact ' . get_bloginfo( 'admin_email' ), 'sumopayments_sagepayform') ) , 'error' );
...
<?php
		...
		function build_query ( $order, $sage_card_number = '', $sage_card_exp_month = '', $sage_card_exp_year = '', $sage_card_cvc = '', $sage_card_type = '', $sage_card_save_token = false, $sage_card_token = false ) {
		    ...
		    $shipping_state 		= $this->check_shipping_address( $order, 'shipping_state' );
			...
			$shipping_last_name 	= apply_filters( 'woocommerce_sagepay_direct_deliverysurname', $this->check_shipping_address( $order, 'shipping_last_name' ), $order );
			$shipping_first_name 	= apply_filters( 'woocommerce_sagepay_direct_deliveryfirstname', $this->check_shipping_address( $order, 'shipping_first_name' ), $order );
			$shipping_address_1 	= apply_filters( 'woocommerce_sagepay_direct_deliveryaddress1', $this->check_shipping_address( $order, 'shipping_address_1' ), $order );
			$shipping_address_2 	= apply_filters( 'woocommerce_sagepay_direct_deliveryaddress2', $this->check_shipping_address( $order, 'shipping_address_2' ), $order );
			$shipping_city 			= apply_filters( 'woocommerce_sagepay_direct_deliverycity', $this->check_shipping_address( $order, 'shipping_city' ), $order );
			$shipping_postcode 		= apply_filters( 'woocommerce_sagepay_direct_deliverypostcode', $this->check_shipping_address( $order, 'shipping_postcode' ), $order );
			$shipping_country 		= apply_filters( 'woocommerce_sagepay_direct_deliverycountry', $this->check_shipping_address( $order, 'shipping_country' ), $order );
        ...
        function sagepay_post( $data, $url ) {
			...
			if( is_wp_error( $res ) ) {
				 ...
				 */
  				if ( $this->debug == true ) {
  					WC_Sagepay_Common_Functions::sagepay_debug( $res->get_error_message(), $this->id, __('Remote Post Error : ', 'sumopayments_sagepayform'), FALSE );
				}
				 ...
				 */
				if ( $this->debug == true ) {
  					WC_Sagepay_Common_Functions::sagepay_debug( $res['body'], $this->id, __('SagePay Direct Return : ', 'sumopayments_sagepayform'), FALSE );
				}
       		...
       		switch( strtoupper( $sageresult['Status'] ) ) {
                case 'OK':
               	...
               	case 'AUTHENTICATED':
					...
					if( isset($sageresult['TxAuthNo']) ){
						update_post_meta( $order_id, '_TxAuthNo' , $sageresult['TxAuthNo'] );
						update_post_meta( $order_id, '_RelatedTxAuthNo' , $sageresult['TxAuthNo'] );
					}

					update_post_meta( $order_id, '_RelatedVPSTxId' , str_replace( array('{','}'),'',$sageresult['VPSTxId'] ) );
					update_post_meta( $order_id, '_RelatedSecurityKey' , $sageresult['SecurityKey'] );

					update_post_meta( $order_id, '_AVSCV2' , $sageresult['AVSCV2'] );
					update_post_meta( $order_id, '_AddressResult' , $sageresult['AddressResult'] );
					update_post_meta( $order_id, '_PostCodeResult' , $sageresult['PostCodeResult'] );
					update_post_meta( $order_id, '_CV2Result' , $sageresult['CV2Result'] );
					update_post_meta( $order_id, '_3DSecureStatus' , $sageresult['3DSecureStatus'] );

					if ( !class_exists('WC_Pre_Orders') && ( $sageresult['Status'] === 'AUTHENTICATED' || $sageresult['Status'] === 'REGISTERED' ) ) {
						update_post_meta( $order_id, '_SagePayDirectPaymentStatus' , $sageresult['Status'] );
					}
                    ...
                    if( isset( $sageresult['FraudResponse'] ) && ( $sageresult['FraudResponse'] === 'DENY' || $sageresult['FraudResponse'] === 'CHALLENGE' ) ) {
						...
						$order->update_status( 'fraud-screen', _x( 'Sage Fraud Response ', 'sumopayments_sagepayform' ) . $sageresult['FraudResponse'] . _x( '. Login to MySagePay and check this order before shipping.', 'sumopayments_sagepayform' ) );
                ...
                case 'ERROR':
                	 ...
                	 * Payment Failed!
                	 */
					$order->add_order_note( __('Payment failed', 'sumopayments_sagepayform') . '<br />' .
													$sageresult['StatusDetail']
												);

					$this->sagepay_message( (__('Payment error', 'sumopayments_sagepayform') . ': ' . $sageresult['StatusDetail'] ) , 'error' );
				...
				case 'REJECTED':
                	 ...
                	 * Payment Failed!
                	 */
					$order->add_order_note( __('Payment failed, there was a problem with 3D Secure', 'sumopayments_sagepayform') . '<br />' .
													$sageresult['StatusDetail']
												);

					$this->sagepay_message( (__('Payment error, there was a problem with 3D Secure', 'sumopayments_sagepayform') . ': ' . $sageresult['StatusDetail'] ) , 'error' );
                ...
                default:
                	 ...
                	 * Payment Failed!
                	 */
					$order->add_order_note( __('Payment failed, contact Sage. This transaction returned no status, you should contact Sage. ' . $sageresult['StatusDetail'], 'sumopayments_sagepayform') );
		 ...
		 * @return [type]          [description]
		 */
		function sumosagepay_message( $message, $type ) {
		 ...
		 */
    	function sumosagepay_credit_card_form() {
...
?>
		 ...
		 */
		function sumosagepaydirect_scripts() {
			wp_enqueue_style( 'wc-sagepaydirect', SUMO_SAGEPLUGINURL.'assets/css/checkout.css' );

			if ( ! wp_script_is( 'jquery-payment', 'registered' ) ) {
				wp_register_script( 'jquery-payment', SUMO_SAGEPLUGINURL.'assets/js/jquery.payment.js', array( 'jquery' ), '1.0.2', true );
			}

			if ( ! wp_script_is( 'wc-credit-card-form', 'registered' ) ) {
				wp_register_script( 'wc-credit-card-form', SUMO_SAGEPLUGINURL.'assets/js/credit-card-form.js', array( 'jquery', 'jquery-payment' ), WC()->version, true );
			}
		...
		public function tokenization_script() {
			wp_enqueue_script(
                ...
                SUMO_SAGEPLUGINURL.'assets/js/tokenization-form.js',
				array( 'jquery' ),
				WC()->version
			);
		}
    	...
    	function process_refund( $order_id, $amount = NULL, $reason = '' ) {
			...
			if ( 'OK' != $result['Status'] ) {
				...
				$order->add_order_note( __('Refund successful', 'sumopayments_sagepayform') . '<br />' .
										__('Refund Amount : ', 'sumopayments_sagepayform') . $amount . '<br />' .
										__('Refund Reason : ', 'sumopayments_sagepayform') . $reason . '<br />' .
										__('Full return from SagePay', 'sumopayments_sagepayform') . '<br />' .
										$refund_ordernote );
         ...
         * @return [type] [description]
         */
        function sumosagepay_register_token( $CardHolder, $CardNumber, $ExpiryDate, $CV2, $CardType ) {
            /**
             * Sent to :
         ...
         */
        function check_shipping_address( $order, $field ) {
