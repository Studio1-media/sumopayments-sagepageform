<?php
     ...
     */
    class WC_SUMO_Sagepay_Common_Functions {
        ...
        public static function sagepay_debug( $tolog = NULL, $id, $message = NULL, $start = FALSE ) {
            ...
            if( $start ) {
                ...
                $logger->log->add( $id, __('', 'sumopayments_sagepayform') );
                $logger->log->add( $id, __('=============================================', 'sumopayments_sagepayform') );
                $logger->log->add( $id, __('', 'sumopayments_sagepayform') );
                $logger->log->add( $id, __('SagePay Log', 'sumopayments_sagepayform') );
                $logger->log->add( $id, __('' .date('d M Y, H:i:s'), 'sumopayments_sagepayform') );
                $logger->log->add( $id, __('', 'sumopayments_sagepayform') );
        ...
        public static function sagepay_basket( $order_id ) {
                ...
                foreach ( $order->get_items() as $item ) {
                    if ( $item['qty'] ) {
                        ...
                        if ( $meta_display ) {
                            ...
                            $meta_output     = apply_filters( 'sagepay_include_meta', $meta_output, $_product, $item );
            ...
            if ( $total_shipping ) {
                $sagepay_basket[] =
                    __( 'Shipping', 'sumopayments_sagepayform' )                                                                 // Description
                    . ':' . 1                                                                                                   // Quantity
            ...
            if ( self::wc_version() < '2.3' ) {
                ...
                $sagepay_basket[] =
                    __( 'Discount', 'sumopayments_sagepayform' )                                                                 // Description
                    . ':' . 1                                                                                                   // Quantity
                ...
                foreach ( $order->get_items() as $item ) {
                    if ( $item['qty'] ) {
                        ...
                        if ( $meta = $item_meta->display( true, true ) ) {
                            // Add a filter to allow $meta to be removed, makes the XML cleaner.
                            $meta           = apply_filters( 'sagepay_include_meta', $meta, $_product, $item );
                            $item['name']   = $item['name'] . ' ' . $meta;
                        }
        ...
        public static function get_icon( $cardtypes, $sagelink, $sagelogo, $id) {
            ...
            if ( ! empty( $cardtypes ) ) {
                ...
                if ( get_option('woocommerce_force_ssl_checkout')=='no' ) {
                    ...
                    foreach ( $cardtypes as $card_type ) {
                        $icon .= '<img src="' . esc_url( SUMO_SAGEPLUGINURL . 'assets/images/card-' . strtolower( str_replace(' ','-',$card_type) ) . '.png' ) . '" alt="' . esc_attr( strtolower( $card_type ) ) . '" />';
                    }
                    ...
                    // display icons for the selected card types
                    foreach ( $cardtypes as $card_type ) {
                        $icon .= '<img src="' . esc_url( WC_HTTPS::force_https_url( SUMO_SAGEPLUGINURL ) . 'assets/images/card-' . strtolower( str_replace(' ','-',$card_type) ) . '.png' ) . '" alt="' . esc_attr( strtolower( $card_type ) ) . '" />';
                    }
                ...
                if ( get_option('woocommerce_force_ssl_checkout')=='no' ) {
                    ...
                    $icon = '<img src="' . esc_url( SUMO_SAGEPLUGINURL . 'assets/images/cards.png' ) . '" alt="' . __( 'Credit card logos', 'sumopayments_sagepayform' ) . '" />';
                } else {
                    // use icon provided by filter
                    $icon = '<img src="' . esc_url( WC_HTTPS::force_https_url( SUMO_SAGEPLUGINURL . 'assets/images/cards.png' ) ) . '" alt="' . __( 'Credit card logos', 'sumopayments_sagepayform' ) . '" />';
                }
            ...
            if ( $sagelogo == '1' ) {
                ...
                if( $sagelink == '1' ) {
                    ...
                    if ( get_option('woocommerce_force_ssl_checkout')=='no' ) {
                        // use icon provided by filter
                        $icon = $icon . sprintf( '<a href="%1$s" onclick="javascript:window.open(\'%1$s\',\'What is SagePay\',\'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700\'); return false;" title="' . esc_attr__( 'What is SagePay?', 'sumopayments_sagepayform' ) . '">' . '<img src="' . esc_url( SUMO_SAGEPLUGINURL . 'assets/images/sagepaylogo.png' ) . '" alt="Payments By SagePay" />' . '</a>', esc_url( self::link() ) );
                    } else {
                        // use icon provided by filter
                        $icon = $icon . sprintf( '<a href="%1$s" onclick="javascript:window.open(\'%1$s\',\'What is SagePay\',\'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700\'); return false;" title="' . esc_attr__( 'What is SagePay?', 'sumopayments_sagepayform' ) . '">' . '<img src="' . esc_url( WC_HTTPS::force_https_url( SUMO_SAGEPLUGINURL . 'assets/images/sagepaylogo.png' ) ) . '" alt="Payments By SagePay" />' . '</a>', esc_url( self::link() ) );
                    }
                    ...
                    if ( get_option('woocommerce_force_ssl_checkout')=='no' ) {
                        // use icon provided by filter
                        $icon = $icon . '<img src="' . esc_url( SUMO_SAGEPLUGINURL . 'assets/images/sagepaylogo.png' ) . '" alt="Payments By SagePay" style="float: right;"/>';
                    } else {
                        // use icon provided by filter
                        $icon = $icon . '<img src="' . esc_url( WC_HTTPS::force_https_url( SUMO_SAGEPLUGINURL . 'assets/images/sagepaylogo.png' ) ) . '" alt="Payments By SagePay" style="float: right;"/>';
                    }
        ...
        function sage_manage_edit_shop_order_columns( $columns ) {
            add_filter( 'manage_edit-shop_order_columns', 'sumo_security_check_column_admin_init' );
        }
        ...
        function security_check_admin_init( $column ) {
            ...
            if ( $column === 'sage_security' ) {
                ...
                if( '' !== get_post_meta( $post->ID, '_AddressResult', TRUE ) ) {
                    ...
                    printf( '<span class="%s tips" data-tip="%s">%s</span>', $addressclass,__( 'Address check ', 'sumopayments_sagepayform' ) . strtolower( $AddressResult ), 'A' );
                ...
                if( '' !== get_post_meta( $post->ID, '_PostCodeResult', TRUE ) ) {
                    ...
                    printf( '<span class="%s tips" data-tip="%s">%s</span>', $postcodeclass,__( 'Postcode check ', 'sumopayments_sagepayform' ) . strtolower( $PostCodeResult ), 'P' );
                ...
                if( '' !== get_post_meta( $post->ID, '_CV2Result', TRUE ) ) {
                    ...
                    printf( '<span class="%s tips" data-tip="%s">%s</span>', $cv2class,__( 'CV2 check ', 'sumopayments_sagepayform' ) . strtolower( $CV2Result ), 'C' );
                ...
                if( '' !== get_post_meta( $post->ID, '_3DSecureStatus', TRUE ) ) {
                    ...
                    printf( '<span class="%s tips" data-tip="%s">%s</span>', $secureclass,__( '3D secure check ', 'sumopayments_sagepayform' ) . strtolower( $SecureStatus ), '3' );
        ...
        function admin_scripts( $hook ) {
            ...
            wp_enqueue_style( 'sagepay-admin-wp', SUMO_SAGEPLUGINURL.'assets/css/admin-css.css' , array(), $this->version );

        }

    }

    $WC_Sagepay_Common_Functions = new WC_SUMO_Sagepay_Common_Functions();

    function sumo_security_check_column_admin_init( $columns ) {
        global $woocommerce;

        $columns["sage_security"] = __( 'Checks', 'sumopayments_sagepayform' );
