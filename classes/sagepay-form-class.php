<?php
     ...
     */
    class WC_Gateway_SUMO_Sagepay_Form extends WC_Payment_Gateway {
		/**
        ...
        public function __construct() {
            ...
            $this->id                   = 'sumosagepayform';
            $this->method_title         = __( 'SUMO SagePay Form', 'sumopayments_sagepayform' );
			...
			$this->default_title 				= __( 'Credit Card via SagePay', 'sumopayments_sagepayform' );
			$this->default_description  		= __( 'Pay via Credit / Debit Card with SagePay secure card processing.', 'sumopayments_sagepayform' );
			$this->default_order_button_text  	= __( 'Pay securely with SagePay', 'sumopayments_sagepayform' );
            ...
            // Disable if mcrypt_encrypt is not installed
            $this->enabled         		= function_exists('mcrypt_encrypt') ? $this->enabled : $this->default_enabled;
			...
			if( $this->enabled == 'yes' ) {
				 ...
				 */
	            add_action( 'woocommerce_api_wc_gateway_sumo_sagepay_form', array( $this, 'check_sagepay_response' ) );

	            add_action( 'valid_sumosagepayform_request', array( $this, 'successful_request' ) );
	            add_action( 'woocommerce_receipt_sumosagepayform', array( $this, 'receipt_page' ) );
	        }
            ...
            if ( $this->status != 'live' ) {
                $this->description .= ' ' . sprintf( __( '<br /><br />TEST MODE ENABLED.<br />In test mode, you can use Visa card number 4929000000006 with any CVC and a valid expiration date or check the documentation (<a href="%s">Test card details for your test transactions</a>) for more card numbers.<br /><br />3D Secure password is "password"', 'sumopayments_sagepayform' ), 'http://www.sagepay.co.uk/support/12/36/test-card-details-for-your-test-transactions' );
        ...
        public function generate_sagepay_form( $order_id ) {
            ...
            wc_enqueue_js('
				jQuery("body").block({
					message: "<img src=\"' . esc_url( apply_filters( 'woocommerce_ajax_loader_url', $woocommerce->plugin_url() . '/assets/images/select2-spinner.gif' ) ) . '\" alt=\"Redirecting&hellip;\" style=\"float:left; margin-right: 10px;\" />'.__('Thank you for your order. We are now redirecting you to SagePay to make payment.', 'sumopayments_sagepayform').'",
					overlayCSS:
					{
						background: "#fff",
						opacity: 0.6
            ...
            // Build VendorTXCode
            $vendortxcode = WC_SUMO_Sagepay_Common_Functions::build_vendortxcode( $order, $this->id, $this->vendortxcodeprefix );
		    ...
		    $shipping_country		= apply_filters( 'woocommerce_sagepay_form_deliverycountry', $this->check_shipping_address( $order, 'shipping_country' ), $order );
            ...
            $sage_pay_args_array = array(
                ...
                'SuccessURL'        => apply_filters( 'sumopayments_sagepayform_successurl', $this->successurl, $order_id ),
                'FailureURL'        => apply_filters( 'sumopayments_sagepayform_cancelurl', html_entity_decode( $order->get_cancel_order_url() ), $order_id, $order_key ),
                ...
                'BillingState'      => WC_SUMO_Sagepay_Common_Functions::sagepay_state( $billing_country, $billing_state ),
                ...
                'DeliveryFirstnames'=> apply_filters( 'woocommerce_sagepay_form_deliveryfirstname', $this->check_shipping_address( $order, 'shipping_first_name' ), $order ),
                'DeliverySurname'   => apply_filters( 'woocommerce_sagepay_form_deliverysurname', $this->check_shipping_address( $order, 'shipping_last_name' ), $order ),
                'DeliveryAddress1'  => apply_filters( 'woocommerce_sagepay_form_deliveryaddress1', $this->check_shipping_address( $order, 'shipping_address_1' ), $order ),
                'DeliveryAddress2'  => apply_filters( 'woocommerce_sagepay_form_deliveryaddress2', $this->check_shipping_address( $order, 'shipping_address_2' ), $order ),
                'DeliveryCity'      => apply_filters( 'woocommerce_sagepay_form_deliverycity', $this->check_shipping_address( $order, 'shipping_city' ), $order ),
                'DeliveryState'     => apply_filters( 'woocommerce_sagepay_form_deliverystate', WC_SUMO_Sagepay_Common_Functions::sagepay_state( $shipping_country, $shipping_state ), $order ),
                'DeliveryPostCode'  => apply_filters( 'woocommerce_sagepay_form_deliverypostcode', $this->check_shipping_address( $order, 'shipping_postcode' ), $order ),
            ...
            $basket = WC_SUMO_Sagepay_Common_Functions::get_basket( $this->basketoption, $order_id );
			 ...
			 */
  			if ( $this->debug == true ) {
                WC_SUMO_Sagepay_Common_Functions::sagepay_debug( $sage_pay_args_array, $this->id, __('Sent to SagePay : ', 'sumopayments_sagepayform'), TRUE );
			}
            ...
            foreach( $sage_pay_args_array as $param => $value ) {
				...
				// Remove all the non-english things
     			$value = strtr( $value, WC_SUMO_Sagepay_Common_Functions::unwanted_array() );
            ...
            return  '<form action="' . $sagepayform_adr . '" method="post" id="sagepayform_payment_form">
                    ' . $sagepayform . '
                    ' . $sagepaycrypt . '
                    <input type="submit" class="button-alt" id="submit_sagepayform_payment_form" value="' . __( 'Pay via SagePay', 'sumopayments_sagepayform' ) . '" /> <a class="button cancel" href="' . $order->get_cancel_order_url() . '">' . __( 'Cancel order &amp; restore cart', 'sumopayments_sagepayform' ) . '</a>
                	</form>';
        ...
        function receipt_page( $order ) {
            echo '<p>' . __( 'Thank you for your order, please click the button below to pay with SagePay.', 'sumopayments_sagepayform' ) . '</p>';
        ...
        function check_sagepay_response() {
            ...
            if ( isset( $_GET["crypt"] ) ) {
				 ...
				 */
				if ( $this->debug == true ) {
  					WC_SUMO_Sagepay_Common_Functions::sagepay_debug( $sagepay_return_values, $this->id, __('SagePay Return : ', 'sumopayments_sagepayform'), FALSE );
				}

                if ( isset( $sagepay_return_values['VPSTxId'] ) ) {
                    do_action( "valid_sumosagepayform_request", $sagepay_return_values );
                }
                ...
                if ( $order_status !== 'completed' && $order_status !== 'processing' ) {
                    ...
                    switch ( strtolower( $sagepay_return_values['Status'] ) ) {
                        case 'ok' :
                            ...
                            $order->add_order_note( __('Payment completed', 'sumopayments_sagepayform') . '<br />' . $ordernotes );
                        ...
                        case 'notauthed' :
                        	// Message
                            $order->update_status( 'on-hold', sprintf( __( 'Payment %s via SagePay.', 'sumopayments_sagepayform' ), woocommerce_clean( $sagepay_return_values['Status'] ) ) );
                        break;
						case 'authenticated' :
                            // Payment authorized
                            update_post_meta( $order_id, '_VPSTxId' , str_replace( array('{','}'),'',$sagepay_return_values['VPSTxId'] ) );
                            ...
                            $order->add_order_note( __('SagePay payment authenticated - No funds have been collected at this time, please log in to yout SagePay control panel to collect the funds', 'sumopayments_sagepayform') . '<br />' . $ordernotes );
                            $order->payment_complete( $sagepay_return_values['VPSTxId'] );
                        ...
                        case 'registered' :
                        	// Message
                            $order->update_status( 'on-hold', sprintf( __( 'Payment %s via SagePay.', 'sumopayments_sagepayform' ), woocommerce_clean( $sagepay_return_values['Status'] ) ) );
							$order->add_order_note( __( 'SagePay payment registered - 3D Secure check failed', 'sumopayments_sagepayform' ) );
                        break;
                        case 'malformed' :
                        case 'invalid' :
                        case 'abort' :
                        ...
                        case 'error' :
                        	// Failed order
                            $order->update_status('failed', sprintf( __( 'Payment %s via SagePay.', 'sumopayments_sagepayform' ), woocommerce_clean( $sagepay_return_values['Status'] ) ) );
                        break;
                    }
                }
        ...
        function sagepay_system_status() {
        	...
        	$description = __( 'SagePay Form works by sending the user to <a href="http://www.sagepay.com">SagePay</a> to enter their payment information.', 'sumopayments_sagepayform' );
